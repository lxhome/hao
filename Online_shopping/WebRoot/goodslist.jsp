  ...
  <div >
   ...
   <div id="gl5" ><table width="550" border="0">
					<tr>
						...
						<font color="red">商品名：</font><%= gs.getG_name()%>
						<br>
						<%-- <font color="red">商品简介:</font><%= gs.getG_infor()%> --%>
							<br>
						<font color="red">商品数量：</font><%=gs.getG_amount() %>件
							</th>
					</tr>
				</table></div>			
   <div id="gl5" ><table width="550" border="0">
					<tr>
						<th width="352" height="143" align="center">
						一口价:￥<%=gs.getG_price() %>
							</th>
					</tr>
				</table>
	...
	<div id="gsl">	
		<!-- 选框栏 -->
			<% int pageNow = Integer.parseInt((String)request.getAttribute("pageNow"));//当前所在页
           int pageCount = Integer.parseInt((String)request.getAttribute("pageCount"));//总页数
//System.out.println(pageNow+ "qwe"+pageCount);
              if(pageNow>1){
                 %>
                 <a href="GoodsServlet?flag=showlist&pageNow=<%=pageNow-1 %>&type=<%=type%>">上一页</a>
                 <%
    ...
    int temp=pageNow+1;
         %>
         <a href="GoodsServlet?flag=showlist&pageNow=<%=temp%>&type=<%=type%>">下一页</a>
         <%
    }          
%>               
</div>				
